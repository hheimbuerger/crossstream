...
export class Scrubber {
    ...
    setupEventListeners() {
        ...
        this.boundHandleMouseEnter = (e) => this.handleMouseEnter(e);
        this.boundHandleMouseMove = (e) => this.handleMouseMove(e);
        this.boundHandleMouseLeave = (e) => this.handleMouseLeave(e);

        // Click to seek
        this.scrubberElement.addEventListener('click', this.boundHandleClick);

        // Hover for thumbnails
        this.scrubberElement.addEventListener('mouseenter', this.boundHandleMouseEnter);
        this.scrubberElement.addEventListener('mousemove', this.boundHandleMouseMove);
        this.scrubberElement.addEventListener('mouseleave', this.boundHandleMouseLeave);
    }
    ...
    setupBusListeners() {
        // Store bound methods for proper cleanup
        this.boundTimeUpdateHandler = (playhead, duration) => {
        ...
        this.boundStateChangeHandler = ({ playhead, duration }) => {
        ...
        bus.on('timeUpdate', this.boundTimeUpdateHandler);

        // Listen for state changes
        bus.on('stateChange', this.boundStateChangeHandler);
    }
    ...
    updateMarkerLabels(e) {
        if (!this.duration || !this.markerLabel) return;
     ...
     */
    destroy() {
        // Remove DOM event listeners if they exist
        if (this.scrubberElement) {
            if (this.boundHandleClick) {
                this.scrubberElement.removeEventListener('click', this.boundHandleClick);
            }
            if (this.boundHandleMouseEnter) {
                this.scrubberElement.removeEventListener('mouseenter', this.boundHandleMouseEnter);
            }
            if (this.boundHandleMouseMove) {
                this.scrubberElement.removeEventListener('mousemove', this.boundHandleMouseMove);
            }
            if (this.boundHandleMouseLeave) {
                this.scrubberElement.removeEventListener('mouseleave', this.boundHandleMouseLeave);
            }
        }

        // Remove bus event listeners with proper callbacks
        if (this.boundTimeUpdateHandler) {
            bus.off('timeUpdate', this.boundTimeUpdateHandler);
        }
        if (this.boundStateChangeHandler) {
            bus.off('stateChange', this.boundStateChangeHandler);
        }

        // Remove all children safely
        if (this.scrubberElement) {
            while (this.scrubberElement.firstChild) {
                this.scrubberElement.removeChild(this.scrubberElement.firstChild);
            }
        }
        ...
        this.boundHandleMouseEnter = null;
